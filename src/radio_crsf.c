...
static crsf_packet_ret_t crsf_check_msg(uint8_t const *const input, uint8_t const frameLength)
{
    ...
    case CRSF_FRAMETYPE_SUBSET_RC_CHANNELS_PACKED: {
        ...
        uint8_t const numOfChannels = ((frameLength - CRSF_FRAME_HEADER_BYTES - 1) * 8) / channelBits;
...
crsf_packet_ret_t crsf_parse_byte(uint8_t inChar)
{
    ...
    if (CRSFframeActive == 0) {
        if (inChar == CRSF_ADDRESS_CRSF_RECEIVER || inChar == CRSF_ADDRESS_CRSF_TRANSMITTER || inChar == CRSF_SYNC_BYTE ||
            inChar == CRSF_ADDRESS_FLIGHT_CONTROLLER || inChar == CRSF_ADDRESS_BROADCAST) {
            CRSFframeActive = 1;
            SerialInPacketLen = 0;
        } else {
            SerialInPacketPtr = 0;
...
void crsf_get_rc_data(uint16_t *const rc_data, uint8_t len)
{
    ...
    for (iter = 0; iter < len && iter < len(_channels); iter++) {
        rc_data[iter] = MAP_U16(_channels[iter], _min, _max, ANALOG_MIN, ANALOG_MAX);
    }
}
